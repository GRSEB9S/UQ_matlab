function [stockdis,stockvel] = FEM_det(T,Method_TM2)
%
...
materialprops = [2.6e10;0.35;2.7e3;0]; %aluminum
L = 10;
w = 1;
nx = 10; %mesh the beam
ny = 3; 
...
ndof = 2;
nnode = (nx+1)*(ny+1);
coords = zeros(2,nnode);
connect = zeros(4,nelem);
for j = 1:ny+1
    for i = 1:nx+1
        coords(1,i+(j-1)*(nx+1)) = (i-1)*L/nx;
        coords(2,i+(j-1)*(nx+1)) = (j-1)*w/ny;
    end
end
for j = 1:ny
    for i = 1:nx
        connect(:,i+(j-1)*nx) = [i+(j-1)*(nx+1);i+1+(j-1)*(nx+1);i+1+j*(nx+1);i+j*(nx+1)];
    end
end
maxnodes = 4;
nelnodes = 4*ones(nelem,1);
elident = ones(nelem,1);

%boundary conditions
nfix = 2*(ny+1); %fixed at the left end
fixnodes = zeros(3,2*ny+2);
for i = 0:ny
    fixnodes(1,2*i+1) = i*(nx+1)+1;
    fixnodes(2,2*i+1) = 1;
    fixnodes(3,2*i+1) = 0;
    fixnodes(1,2*i+2) = i*(nx+1)+1;
    fixnodes(2,2*i+2) = 2;
    fixnodes(3,2*i+2) = 0;
end
ndload = 1;
dloads = [nelem;2;0;1e5];

%time depedent part of the force
dt = T(2)-T(1);
szt = length(T);
t_final = T(szt);
time_dep = zeros(1,szt);
a = 3/t_final;
b = 20*pi/t_final;
for i = 1:szt
...
%
  ...
  nprint = 20;
     ...
     dis0 = zeros(ndof*nnode,1);
     vel0 = zeros(ndof*nnode,1);

     alpha1 =0; alpha2 = 10;
     C = alpha1 * K + alpha2 * M;
     for i = 1:szt
         newF(:,i)=F*time_dep(1,i);
     end
     tic;
     [stockdis,stockvel,stockacc] = Second_solver(Method_TM2,dis0,vel0,dt,szt,K,C,M,newF,0);
     time_second = toc;
     str = strcat('Time for second order solver is ',num2str(time_second,3),' (s)\n');
     fprintf(str);
     
     %transform the second order ODE to first order ODE, solver using the First_solver
%      cellM{1}=M;cellK{1}=K;cellC{1}=C;cellF{1}=newF;
%      [A,B,P] = Transform_first(cellM,cellK,cellC,cellF);
%      A=A{1};B=B{1};P=P{1};
%      ticID = tic;
%      T = 0:dt:dt*(szt-1);
%      stockX = First_solver([vel0;dis0],T,Method_TM1,A,B,P);
%      time_first = toc(ticID);
%      str = strcat('Time for first order solver is ',num2str(time_first,3),' (s)\n');
%      fprintf(str);
     ...
     for n = 1:szt
         if (count == nprint)
             ...
             for i = 1 : nnode
                 for j = 1 : ndof
                     defcoords(j,i) = coords(j,i) + scale_factor*stockdis(ndof*(i-1)+j,n);
                 end
             end
             clf
             axis([0,11,-5.5,5.5])
             plotmesh(defcoords,ncoord,nnode,connect,nelem,elident,nelnodes,'r');
             pause(0.02/(szt/nprint));
             count = 0;
         end
         count = count + 1;
     end
